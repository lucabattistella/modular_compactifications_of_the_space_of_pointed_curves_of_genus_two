...
\begin{rem}
 ...
 \[\mathcal W=\{(C,p)|\ C\text{ smooth and } p \text{ Weierstrass}\}\subseteq\oM_{2,1},\]
 ...
 \includegraphics[width=.8\textwidth]{admissible_conjugate}  
  \end{center}
 \end{itemize}
 We observe here that in the case (pseudocompact, or genus one with rational bridge), the singularity of the total space of a smoothing $\mathcal C\to\dvr$ at the two distinguished nodes (separating the elliptic component from the rational chain) are both $A_k$ for the same $k$, because they map to the same node of the target in the admissible cover picture. This fact is stable under base change, and it determines a symmetry of the rational chain in the model with regular total space.
\end{rem}


\begin{prop}\label{prop:tailI}
 Let $\phi\colon\mathcal C\to\overline{\mathcal C}$ be a contraction over the spectrum of a discrete valuation ring $\dvr$, where: $\mathcal C\to \dvr$ is a family of prestable (reduced, nodal) curves of arithmetic genus two, with regular total space and smooth generic fiber $\mathcal C_{\eta}$; and $\overline{\mathcal C}\to\dvr$ is a family of Gorenstein curves of arithmetic genus two, with $\overline{\mathcal C}_{\eta}$ smooth, and $\overline{\mathcal C}_0$ containing a genus two singularity of type $I_m$. Denote by $(Z;q_1,\ldots,q_m)$ the exceptional locus of $\phi$ marked with $Z\cap\overline{\mathcal C_0\setminus Z}$, where $q_1$ corresponds to the special branch of $\overline{\mathcal C}_0$. Then:
 \begin{enumerate}[leftmargin=.6cm]
  \item The stabilisation of $(Z,q_1)$ is Weierstrass.
  \item Let $r_1$ be the point of the core of $Z$ closest to $q_1$, and let $k$ be the length of $R_1$, the rational chain separating $q_1$ from $r_1$. With similar notation, $R_i$ has length 
  \begin{equation*}
  \begin{cases}
   3k+1+\dist(r_1,r_i) & \text{if } r_i\neq r_1,\\
   k+2\dist(q_1,\tilde r_i)  & \text{if } r_i=r_1
  \end{cases} 
  \end{equation*}
 ($i=2,\ldots,m$), where $\tilde r_i$ is the point of $R_1$ closest to $q_i$, and $\dist(a,b)$ is the number of irreducible components between the points $a$ and $b$ (so for example it is $1$ if $a$ and $b$ lie on the same irreducible component).
 \end{enumerate}
\end{prop}

\begin{prop}\label{prop:tailII}
 Same as above with $\overline{\mathcal C}_0$ containing a genus two singularity of type $I\!I_m$, and $q_1,q_2$ corresponding to the special branches. Then:
 \begin{enumerate}[leftmargin=.6cm]
  \item The stabilisation of $(Z,q_1,q_2)$ is conjugate.
  \item $R_1$ and $R_2$ have the same length $k$, and $R_i$ has length 
  \begin{equation*}
  \begin{cases}
   2k+\min_{\epsilon\in\{1,2\}}\dist^*(r_\epsilon,r_i) & \text{if } r_1\neq r_2, \text{ and } r_i\notin\{r_1,r_2\},\\
   k+\dist(q_1,\tilde r_i)  & \text{if } r_1\neq r_2, \text{ and } r_i=r_1 (\text{and symm. } 1\leftrightarrow 2), \\
   2k+\dist(r_1,\tilde r_2)+\dist(r_1,r_i) & \text{if } r_1= r_2, \text{ and } r_i\neq r_1, \\
   k+\dist(q_1,\tilde r_i)+\delta(\dist(\tilde r_2,\tilde r_i)-1)& \text{if } r_1= r_2=r_i, \text{ and } \delta=\begin{cases} 1 & \tilde r_2\in[q_1,\tilde r_i] \\ 0 & \text{otherwise} \end{cases}
  \end{cases} 
  \end{equation*}
 ($i=2,\ldots,m$), where we set $\dist^*(r_\epsilon,r_i)=1$ if the core consists of a genus one curve with a rational bridge, and $r_i$ lies between $r_1$ and $r_2$ on this rational bridge, and $\dist^*(r_\epsilon,r_i)=\dist(r_\epsilon,r_i)$ otherwise.
 \end{enumerate}
\end{prop}
