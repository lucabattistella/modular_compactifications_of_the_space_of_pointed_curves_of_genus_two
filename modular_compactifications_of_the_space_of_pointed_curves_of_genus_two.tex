...
\begin{rem}
 We suspect that another incarnation of the crimping space is in the moduli of arrows $\phi$ as in the diagram of Lemma \ref{lem:aut}, satisfying a number of requirements. We will support this with two simple computations. We note that $H^0(\Omega_{\PP^1}^\vee(-p)_{|2p})$ is the tangent space to the subgroup of automorphisms of $\PP^1$ fixing one point $p$, so it is isomorphic as a Lie algebra to the only non-abelian Lie algebra of dimension two $V$. It has a basis $e_1=\begin{pmatrix} 1 & 0 \\ 0 & -1\end{pmatrix}$, $e_2=\begin{pmatrix} 0 & 0 \\ 1 & 0\end{pmatrix}$ with $[e_1,e_2]=-2e_2$. The vector $(\varphi,\psi)$ therefore corresponds to the infinitesimal automorphism: \[t\mapsto\frac{1+\epsilon\varphi t}{1-\epsilon(\varphi t+\psi)}=t+\epsilon(2\varphi t-\psi t^2).\]
 We require $\phi$ to be the embedding (i.e. a point of $\operatorname{Gr}(m,V^{\oplus m})$) of a Lie subalgebra, such that the corresponding group of infinitesimal automorphisms fixes uniquely the subalgebra of a singularity of genus two in $\k[[t_1]]\oplus\ldots\k[[t_m]]$.
 
 The unibranch case goes as follows: the subalgebra of $\k[[t]]$ generated by $x=t^2+ct^3$ is preserved by $(\varphi,\psi)$ iff
 \[(1+2\varphi)^2t^2-2\psi(1+2\varphi)t^3+c(1+2\varphi)^3t^3\text{ is a multiple of } t^2+ct^3,\]
 which reduces to $\varphi(1+2\varphi)c=\psi$. This furthermore determines $c$ iff $\varphi\neq 0$. Note that in this case the Lie subalgebra condition is automatically satisfied. We have found $(\varphi,\psi)\in\k^\times\times\k$.
 ...
 \end{align*}
 which are also automatically satisfied after \eqref{eqn:vanishing_of_minors}. We see the various equations above cut inside $\PP^5_{[w_{ij}]}$ the locus \[(\Aaff^1_{{w_{14}}/{w_{12}}}\setminus\{0\})\times\Aaff^1_{{w_{24}}/{w_{12}}}.\]
\end{rem}
...
Following the previous discussion, there is a more geometric way to realise the dicotomy between the atom and the non-atom. The non-Gorenstein genus one singularity of type $\sigma_0^{II}$ (resp. $\sigma_0^{III}$), with every branch rational and one-pointed, has automorphism group $\Gm^{m-1}$ (resp. $\Gm^m$); the latter group therefore acts on the tangent space at the singular point, and it can be checked that of the lines fixed by this action only one sits inside the open subset corresponding to the crimping space, while all other lines are identified under the group action - by collapsing them, they give rise to the atom and non-atom respectively.
