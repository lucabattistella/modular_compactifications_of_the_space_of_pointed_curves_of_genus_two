...
We construct alternative compactifications of the moduli stack of smooth $n$-pointed curves of genus two. The boundary of the Deligne-Mumford compactification, consisting of stable nodal curve, is gradually replaced by ever more singular curves, complying with more restrictive combinatorial requisites on the dual graph. For $1\leq m <n$, the notion of $m$-stability allows genus one and two singularities, while demanding that every higher genus subcurve contain at least a number of special points. Our main result on the stack of $m$-stable curves is:
\begin{teo*}
 $\oM^{(m)}_{2,n}$ is a \emph{proper} irreducible Deligne-Mumfod stack over $\operatorname{Spec}(\mathbb Z[\frac{1}{30}])$.
\end{teo*}
This paper fits into the context of birational geometry of moduli spaces, extending work of D.I. Smyth in genus one, but we also expect it to find applications in enumerative geometry. We classify singularities of genus two and their (semi)stable models, highlighting the relation with Brill-Noether theory. We interpret crimping spaces as moduli spaces for the differential geometric data needed in order to construct a higher genus singularity from an ordinary $m$-fold point, and verify their impact on the existence of automorphisms, a phenomenon that had not fully emerged in lower genus.
...
\end{lem}
\begin{proof}
 Let $K(\tilde C)$ denote the constant sheaf of rational functions on $\tilde C$. A section of $\Omega_{\tilde C}^\vee\otimes K(\tilde C)$ is contained in $\Omega_C^\vee$ if and only if its image under push-forward
...
\begin{description}
 \item[$A_4$] In the coordinates $x=t^2+ct^3,y=t^4,z=t^5$ (they are redundant, but this will not cause any problem), the section $f(t)\frac{d}{dt}\in\nu_*\Omega_{\tilde C}^\vee\otimes K(\tilde C)$ pushes forward to
...
\begin{rem}\label{rmk:Wandconj}
 ...
 \caption{Admissible covers and conjugate points.}\label{fig:adm_conj}
  \end{figure}
 \end{itemize}
 We observe that in case (\dag), the singularity of the total space of a smoothing $\mathcal C\to\dvr$ at the two distinguished nodes (separating the elliptic component from the rational chain) are both $A_k$ for the same $k$, because they map to the same node of the target in the admissible cover. This is stable under base change, so it entails a symmetry of the rational chain in the model with regular total space.
\end{rem}
