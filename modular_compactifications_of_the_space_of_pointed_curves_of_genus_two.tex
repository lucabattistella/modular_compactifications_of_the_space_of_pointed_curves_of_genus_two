...
\begin{prop}[Semistable tails]\label{prop:semistable tails} Let $(C,p)$ be a Gorenstein singularity of genus two, with pointed normalisation $\bigsqcup_{i=1}^m(\PP^1,p_i)$. Let $\mathcal C\to\dvr$ be a one-parameter smoothing of $C$, and $\phi\colon\mathcal C^{ps}\to\mathcal C$ a birational contraction from a prestable curve with regular total space. Let $(Z,p_1,\ldots,p_m)$ be $\phi^{-1}(p)$ marked with the intersection points with the rest of $\mathcal C^{ps}_0$.
 \begin{itemize}[leftmargin=.5cm]
...
\begin{proof}
 ...
 We are going to find conditions on $\mathcal C^{ps}_0$ so that a line bundle of the form $\omega_{\mathcal C^{ps}/\dvr}(D+\Sigma)$ as in Lemma \ref{lem:Gorenstein_contraction} may be found, such that it restricts to the trivial line bundle on $Z$, and it is ample over $\dvr$ everywhere else. Semiampleness will then follow from Lemma \ref{lem:general_contraction} in case $m\geq 3$, and from a case-by-case analysis if $m=1,2$.
 
 By Castelnuovo's criterion, we may assume that $Z$ is semistable, i.e. every rational component contains at least two special points. We may split $Z$ into a core $K$ (minimal subcurve of genus two) and a number (possibly zero) of rational trees. We start by analysing the latter ones. By Lemma \ref{lem:dualising_lb}, in case II both $p_1$ and $p_m$ are attached to components that appear with multiplicity $2$ in $D$ - respectively, in case III, $p_m$ is attached to a component of multiplicity $3$ -, while all other markings lie on components along which $D$ has multiplicity $1$.
...
\begin{proof}
 ...
 Assume now that $Z$ is the union of two curves of genus one, $p\in E_1$ and $E_2$. We need to study $\mathcal L=\omega_{\mathcal C/\dvr}(3E_1+4E_2+\Sigma)$. Observe that $R^1\pi_*\mathcal L(-E_1-2E_2)=0$, therefore we can just as well study $\pi_*\mathcal L_{|E_1+2E_2}$. We have an exact sequence:
  ...
  and noticing that $H^1(\OO_{E_1}(2p-2q))\simeq\k$ (because $Z$ is a Weierstrass tail), it is enough to show that $H^1(\OO_{E_1+2E_2}(2p))\to H^1(\OO_{2E_2})$ has non-trivial kernel. By working locally around $q$, it can be shown that there is an exact sequence:
  ...
  \[0\to \OO_{E_2}(q)\to \OO_{2E_2}\to\OO_{E_2}\to 0\]
