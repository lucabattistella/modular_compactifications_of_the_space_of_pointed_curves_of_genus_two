...
\begin{prop}
 ...
 \begin{enumerate}[leftmargin=.6cm]
  ...
  \end{cases} 
  \end{equation*}
 where $\tilde r_i$ is the point of $R_1$ closest to $q_i$, and $\dist(a,b)$ is the number of irreducible components between the points $a$ and $b$ (so for example it is $1$ if $a$ and $b$ lie on the same irreducible component).
 \end{enumerate}
\end{prop}
...
\begin{prop}
 ...
 \begin{enumerate}[leftmargin=.6cm]
  ...
  \begin{equation*}
  \begin{cases}
   2k+\dist(r_1,r_i) & \text{if } \dist(r_1,r_2)=1 \text{ and } \dist(r_1,r_i)>0,\\
